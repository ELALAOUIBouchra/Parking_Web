...
public class SectionService implements IDao<Section> {
    ...
    public boolean update(Section o) {
        try {
            String req = "update section set code = ?, reference =? where id = ?";
            PreparedStatement pr = Connexion.getConnection().prepareStatement(req);
    ...
    public Section findById(int id) {
        try {
            ...
            if (rs.next()) {
                return new Section(rs.getInt("id"), rs.getString("code"), rs.getString("reference"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Section> findAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                sections.add(new Section(rs.getInt("id"), rs.getString("reference"), rs.getString("code")));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Section> findSectionByReference(String ref) {
        ...
        try {
            ...
            while (rs.next()) {
                sections.add(new Section(rs.getInt("id"), rs.getString("code"), rs.getString("reference")
                ));
            }
